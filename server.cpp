...
long parsePort(char **argv)
{
    long temp_port;
    try {
        temp_port = std::stol(argv[1]);
    } catch (...) {
        printError("Failed to parse port number. It needs to be a valid integer greater than 1023.");
        exit(1);
    }
...
struct sockaddr_in createServerAddr(const int port)
{
    struct sockaddr_in addr;
...
void communicate(int clientSockfd, const string& fileDir, int num)
{
    const int BUF_SIZE = 1024;
    char buf[BUF_SIZE] = {0};
    ofstream fout(getFileName(fileDir, num));
    if (!fout.is_open())
    {
    ...
    while (true)
    {
        ...
        int rec_res = recv(clientSockfd, buf, BUF_SIZE, 0);
        if (rec_res < 0)
        {
            printError("Error in receiving data");
            exitOnError(clientSockfd);
...
int main(int argc, char **argv)
{
    ...
    setReuse(sockfd);
    setupNonBlocking(sockfd);
    
    struct sockaddr_in addr = createServerAddr(args.port);
    
    if (bindSocket(sockfd, addr) == -1) {
        printError("bind() failed");
        exitOnError(sockfd);
    }
